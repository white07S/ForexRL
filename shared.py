...
with open('dataset/y_train.pkl', 'rb') as f: y_train = cPickle.load(f)
with open('dataset/y_test.pkl', 'rb') as f: y_test = cPickle.load(f)
...
with open('dataset/X_train.pkl', 'rb') as f: X_train = cPickle.load(f)
with open('dataset/X_test.pkl', 'rb') as f: X_test = cPickle.load(f)
...
def plot_function(epoch_weights):
    ...
    ret.to_csv('outputs/hourly_returns.csv')
    ...
    plt.savefig('graphs/cumulative_return', bbox_inches='tight', pad_inches=0)
    plt.close()
    ...
    plt.savefig('graphs/weekly_returns', bbox_inches='tight', pad_inches=0)
    plt.close()
    ew_df = pd.DataFrame(ew[1:], index = ind, columns =  assets)
    ew_df.to_csv('outputs/portfolio_weights.csv')
    ...
    ax.text(0.5, 1.0, ttt, horizontalalignment='center', verticalalignment='top', transform=ax.transAxes)
    plt.savefig('graphs/transactions', bbox_inches='tight', pad_inches=0)
    plt.close()
    ew_df = ew_df.resample('1D').mean()
    plt.figure(figsize=(48, 12))
    ax = sns.heatmap(ew_df.T, cmap=cmap, center=0, xticklabels=False, robust=True)
    ax.text(0.5, 1.0, ttt, horizontalalignment='center', verticalalignment='top', transform=ax.transAxes)
    plt.savefig('graphs/portfolio_weights', bbox_inches='tight', pad_inches=0)
    plt.close()